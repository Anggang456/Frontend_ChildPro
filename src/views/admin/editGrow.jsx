...
const editGrow = () => {
  ...
  const [datevac, setDatevac] = useState([]);
  ...
  const handleMidwifeChange = (event) => {
  ...
  const getGrow = async () => {
    const response = await axios.get(`http://localhost:3000/datagrow/${id}`)
    setName(response.data.name);
    setParents(response.data.parents);
    console.log(response.data.parents)
    setStatus(response.data.status);
    setDatevac(response.data.datevac);
    setWeight(response.data.weihgt);
    setHeight(response.data.height);
    setHead(response.data.head);
  }

  const updateGrow = async () => {
    await axios.patch(`http://localhost:3000/datagrow/${id}`, {
      status: status,
      datevac: datevac,
      weight: weight,
      height: height,
    ...
    if(response.status === 200) navigate("/datagrow")
  }
            ...
            <div id="content">
              <nav className="navbar navbar-expand-sm bg-white border-bottom mb-4 topbar static-top navbar-light">
                <div className="container-fluid"><button id="sidebarToggleTop" className="btn btn-link d-md-none rounded-circle me-3" type="button"><i className="fas fa-bars"></i></button>
                  <strong className='text-dark' style={{ fontFamily: "Poppins" }}>Edit status pertumbuhan</strong>
              ...
              <div className='container d-flex justify-content-center rounded-2 p-4 text-dark'>
                <div className='col-md-10 p-4 rounded-2' style={{
                  ...
                  <div className='d-flex justify-content-center mb-5'>
                    <h3>Update Status Pertumbuhan</h3>
                  </div>
                  <form onSubmit={updateGrow}>
                    <div className='row'>
                      <div className='col-md-5'>
                        <div className="form-group py-2">
                          <label htmlFor="name" className='fw-bold'>Nama Orang Tua: *</label>
                          <input type="name" className="form-control" id="name" placeholder={parents} aria-describedby="name" readOnly style={{
                            border: "3px solid rgba(255, 155, 80, 0.98)",
                          }} />
                        </div>
                      </div>
                      <div className='col-md-7'>
                        <div className="form-group py-2">
                          ...
                          <input type="name" className="form-control" id="name" aria-describedby="name" readOnly placeholder={name} style={{
                            border: "3px solid rgba(255, 155, 80, 0.98)",
                          }} />
                        </div>
                      </div>
                      <div className='col-md-4'>
                        <div className="form-group py-2">
                          <label htmlFor="SelectOption" className='fw-bold'>Status: *</label>
                          <select id="selectOption"
                            name="selectOption"
                            value={midwife}
                            onChange={handleMidwifeChange} className="form-select" aria-label="Default select example" style={{
                              border: "3px solid rgba(255, 155, 80, 0.98)",
                            }}>
                      ...
                      <div className='col-md-3'>
                        <div className="form-group py-2">
                          <label htmlFor="datevac" className='fw-bold'>Tanggal Vaksin: *</label>
                          <input type="date" className="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" value={datevac} required onChange={(e) => setDatevac(e.target.value)} style={{
                            border: "3px solid rgba(255, 155, 80, 0.98)",
                          }} />
                        </div>
                      </div>
                      <div className='col-md-5'>
                        <div className="form-group py-2">
                          <label htmlFor="SelectOption" className='fw-bold'>Nama Bidan: *</label>
                          <select id="selectOption"
                            name="selectOption"
                            value={status}
                            onChange={handleStatusChange} className="form-select" aria-label="Default select example" style={{
                              border: "3px solid rgba(255, 155, 80, 0.98)",
                            }}>
                            <option defaultValue>Pilih Bidan</option>
                            
                            <option value="Hilda Martina">Bidan Hilda Martina</option>
                            <option value="Mila">Bidan Mila</option>
                            <option value="Lanny Kuswadi">Bidan Lanny Kuswadi</option>
                            <option value="Ika">Bidan Ika</option>
                          </select>
                        </div>
                      </div>
                      <div className="col-md-4 pt-2">
                        <label htmlFor="weight" className='fw-bold'>Berat Badan: *</label>
                        <div className="input-group py-2">
                          <input type="number" className="form-control" id="inlineFormInputGroup" placeholder="Masukkan tinggi badan" value={weight} required onChange={(e) => setWeight(e.target.value)} style={{
                            border: "3px solid rgba(255, 155, 80, 0.98)",
                          }} />
                          <div className="input-group-prepend bg-transparent rounded-0" style={{
                            border: "3px solid rgba(255, 155, 80, 0.98)",
                          }}>
                            <div className="input-group-text rounded-0 border-0" style={{ backgroundColor: "#FEEFD0" }}>cm</div>
                          </div>
                        </div>
                      </div>
                      <div className="col-md-4 pt-2">
                        <label htmlFor="height" className='fw-bold'>Tinggi Badan: *</label>
                        <div className="input-group py-2">
                          <input type="number" className="form-control" id="inlineFormInputGroup" placeholder="Masukkan tinggi badan" value={height} required onChange={(e) => setHeight(e.target.value)} style={{
                            border: "3px solid rgba(255, 155, 80, 0.98)",
                          }} />
                          <div className="input-group-prepend bg-transparent rounded-0" style={{
                            border: "3px solid rgba(255, 155, 80, 0.98)",
                      ...
                      <div className="col-md-4 pt-2">
                        <label htmlFor="head" className='fw-bold'>Lingkar Kepala: *</label>
                        <div className="input-group py-2">
                          <input type="number" className="form-control" id="inlineFormInputGroup" placeholder="Masukkan lingkar kepala" value={head} required onChange={(e) => setHead(e.target.value)} style={{
                            border: "3px solid rgba(255, 155, 80, 0.98)",
                          }} />
                          <div className="input-group-prepend bg-transparent rounded-0" style={{
                            border: "3px solid rgba(255, 155, 80, 0.98)",
