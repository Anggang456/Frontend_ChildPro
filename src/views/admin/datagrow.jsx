...
const datagrow = () => {
    ...
    const getData = async () => {
      try {
        const response = await axios.get('http://localhost:3000/datagrow', {
          headers: {
    ...
    const deleteGrow = async (id) => {
        try {
            const response = await axios.delete(`http://localhost:3000/datagrow/${id}`);
            getData();
        } catch (error) {
            console.log(error);
        }
                    ...
                    <nav className="navbar align-items-start sidebar sidebar-dark accordion bg-white p-0 navbar-dark border">
                        <div className="container-fluid d-flex flex-column p-0"><a className="navbar-brand d-flex justify-content-center align-items-center sidebar-brand m-0" href="/">
                            ...
                            <ul id="accordionSidebar" className="navbar-nav">
                                ...
                                <li className="nav-item"><a className="nav-link active text-dark" href="/datagrow"><i><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                        ...
                        <div id="content">
                            <nav className="navbar navbar-expand-sm bg-white border-bottom mb-4 topbar static-top navbar-light">
                                <div className="container-fluid"><button id="sidebarToggleTop" className="btn btn-link d-md-none rounded-circle me-3" type="button"><i className="fas fa-bars"></i></button>
                                    <strong className='text-dark' style={{ fontFamily: "Poppins" }}>Data GrowCheck</strong>
                                ...
                                <div className="card-body">
                                    <div className="row">
                                        ...
                                        <div className="col-md-6">
                                            <div id="dataTable_filter" class="text-md-end dataTables_filter"><label className="form-label"><input className="form-control form-control-sm" type="search" aria-controls="dataTable" placeholder="Search" /></label></div>
                                        </div>
                                    </div>
                                            ...
                                            {data.map((item, index) => (
                                                <tr key={item.id}>
                                                    ...
                                                    <td style={{width:'100px'}}>
                                                        <Link to={`/editgrow/${item.id}`}  className='border-0 bg-transparent'><i className='far fa-edit text-primary' style={{fontSize:'24px'}}></i></Link>
                                                        <button onClick={() => deleteGrow(item.id) } className='border-0 bg-transparent'><i className='far fa-trash-alt text-danger' style={{fontSize:'24px'}}></i></button>
                                                    </td>
                                                </tr>
                                                ))}
                                            </tbody>
